...
export class ChatService {
  ...
  async handle_update_lead_email(
    ...
    try {
      ...
      const sparky =
        (await this.user_repository.get_by_id(
          ((updated_message.conversation as Conversations)?.workspace as WorkSpaces)?.spark_gpt_agent?._id,
        )) ??
        (await this.authService.create_spark_gpt_agent(
          (updated_message.conversation as Conversations)?.workspace as WorkSpaces,
        ));
      ...
      const cta_pre_message = await this.messages_repository.create({
        ...
        content: {
          text: 'Thank you for submitting your email, an agent will reply shortly',
        },
        type: MESSAGE_TYPE.TEXT,
        workspace: (updated_message.conversation as Conversations)?.workspace
      });

      client.emit('new_message', {
        data: cta_pre_message,
      })
      ...
      response = { data: null, error: { msg: e.message, status: 500 } };
    }
    ...
    if (client.user.type === USERTYPE.CLIENT) {
      ...
      if (new_message.type === MESSAGE_TYPE.SWITCH_TO_AGENT) {
        ...
        if (!sender.email && (widget_config.email_required == EMAIL_REQUIRED_STATUS.ALWAYS || duringWorkingHours)) {
          ...
          const sparky =
            (await this.user_repository.get_by_id(
              ((final.conversation as Conversations)?.workspace as WorkSpaces)?.spark_gpt_agent?._id,
            )) ??
            (await this.authService.create_spark_gpt_agent(
          ...
          const cta_pre_message = await this.messages_repository.create({
            ...
            content: {
              text: 'In the meantime, could you tell us more about you please?',
            },
            type: MESSAGE_TYPE.TEXT,
          ...
          const cta_pre_message = await this.messages_repository.create({
            ...
            conversation: final.conversation,
            content: {
              text: 'An agent will reply shortly.',
            },
            type: MESSAGE_TYPE.TEXT,
            workspace: (final.conversation as Conversations)?.workspace
          });
